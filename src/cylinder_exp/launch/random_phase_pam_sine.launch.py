...
def generate_launch_description():
    bag_dir = os.path.expanduser(
        f'~/koni_log/random_phase_pam_sine_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    )
    ...
    pam_high_pressure_kpa = LaunchConfiguration('pam_high_pressure_kpa')
    pam_random_seed = LaunchConfiguration('pam_random_seed')
    ...
    pam_high_pressure_param = ParameterValue(
        pam_high_pressure_kpa, value_type=float
    )
    pam_random_seed_param = ParameterValue(pam_random_seed, value_type=int)
    ...
    controller_node = Node(
        ...
        name='random_phase_pam_sine_position_controller_node',
        output='screen',
        parameters=[{
            # AO channels
            'ch_head': 3,
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('pam_high_pressure_kpa', default_value='100.0'),
        DeclareLaunchArgument('pam_random_seed', default_value='0'),
